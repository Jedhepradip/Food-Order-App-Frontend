...
//             const filteredRestaurant = RestaurantData.filter((e) => e?._id === id
//             ) || null;
...
//             {/* Restaurant Banner */}
//             <div
//                 className="relative w-full h-64 bg-cover bg-center rounded-lg shadow-md bg-white overflow-hidden"
//                 style={{
...
const ViewMenuPage: React.FC = () => {
    ...
    useEffect(() => {
        if (RestaurantData && id) {
            const filteredRestaurant = RestaurantData.filter(
                (e): e is RestaurantInterface => e?._id === id
            );
            if (filteredRestaurant) {
        ...
        <div className="bg-black min-h-screen p-6 text-white md:px-32">
            {Restaurant ? (
                <>
                    ...
                    {/* Restaurant Details */}
                    <div className="mt-6 space-y-2">
                        <p className="font-semibold text-2xl">{Restaurant.restaurantName}</p>
                        <p className="text-lg text-white font-bold flex gap-1">
                            <TiStopwatch className="mt-1" /> Delivery Time:{" "}
                            <span className="text-orange-400">{Restaurant.deliveryTime}</span>
                        </p>
                    </div>
                </>
            ) : (
                <p className="text-center text-gray-400">Restaurant not found.</p>
            )}
        </div>
    );
};
